...
2. Establish a command structure (incident lead, forensic owner, comms owner).
3. Contain actively exploited paths first, preserving forensic artifacts before destructive cleanup when feasible.
4. Scope affected identities, services, data stores, and downstream dependencies.
5. Eradicate root access path, rotate exposed credentials, and patch exploited weaknesses.
6. Recover in staged rollout with explicit rollback criteria and heightened monitoring.
7. Publish a post-incident action list with prevention owners and due dates.
